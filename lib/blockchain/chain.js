...
const {Lock} = require('bmutex'); // 互斥锁
const LRU = require('blru'); // "最近最少使用" 缓存库
...
class Chain extends AsyncEmitter {
  ...
  constructor(options) {
    ...
    this.opened = false; // chain打开与否的标记(打开时，即等待数据库的加载)
    this.options = new ChainOptions(options); // 链属性的实例化
    ...
    // connect-to-the-p2p-network.js中，workers为undifined，此时亦可。那是因为：
    // 1. master.js中有默认worker.js文件; 
    // 2. 即便环境不支持workers，this.workers确实不能发挥作用。这时是将逐个任务放入宏任务队列，一个一个的执行）
    this.workers = this.options.workers; 

    this.db = new ChainDB(this.options); // 链数据库的实例化

    this.locker = new Lock(true, BufferMap); // 初始化一个互斥锁()
