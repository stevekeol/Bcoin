...
class WorkerPool extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.enabled = false;
    this.size = getCores(); // cpus数，也是workers的上限数
    this.timeout = 120000;
    /**
     * 倘若没有临时设置worker_file，则使用默认的worker.js
     * @type {[type]}
     */
    this.file = process.env.BCOIN_WORKER_FILE || 'worker.js';

    this.children = new Map(); // 用来维护 workers
    this.uid = 0;
  ...
  spawn(id) {
    /**
     * [错误描述]核心: 其实这里就是苦苦追寻的创建WebWorker的地方
     * [正确描述]TMD! 最终发现此处的Worker是workerpool.js中定义的class Worker。而非w3c标准内置的worker
     * @TODO 此处的new Worker 是否应该替换为内置的Worker(先思考有没有换的必要，其次再考虑要不要换)
     * 感悟代码哲学: 此处只负责创建worker作为一个child. 在绑定监听事件后，返回该child对象。其余一概不管。
     */
    ...
    child.on('exit', (code) => {
      ...
      /**
       * 因为在spawn()内创建的child尚处在监听状态(即存续状态)，因此child对象本身当然存在。
       * 因此是可以比较的。
       */
  ...
  alloc() {
    ...
    if (!this.children.has(id))
      /**
       * 感悟代码设计哲学: 我alloc()函数只负责分配一个可用的child/worker。因此当没有可用worker时，我仅仅只需告诉某个小伙子去创建，我只需要拿来就用
       */
  ...
  execute(packet, timeout) {
    /**
     * 当没有开启workers模式，或不支持该模式时，就将这些packet处理任务，挨个加入宏任务队列，依次执行;
     */
    ...
    /**
     * 感悟其中代码设计的哲学: 只需知道此处我要分配一个工作对象worker(child)出来，其内在分配逻辑我不管。
     */
   ...
   * 诸如check()的最终调用链: workerpool.js - node.js - fullnode.js - Chain/Mempool/Miner - tx.js/checkAsync()中
   * @method
   * @param {TX} tx
   * @param {CoinView} view
...
function getCores() {
  /**
   * 疑问: 创建worker的流程，貌似并没有真正分配到不同的cpu中去呀，还是我没看完代码的缘故？
   */
